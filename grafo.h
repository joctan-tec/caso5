...
class Grafo {
    ...
    public:
        ...
        vector<INodo*> deepPath(INodo* pOrigen) {  //recorrido en profundidad
            vector<INodo*> result;
            int visitados = 0;
            stack<NodoGrafo*> pilaRevisados;
            stack<NodoGrafo*> pilaAdyasentes;

            resetNodes();
            NodoGrafo* puntoPartida = this->getNodo(pOrigen->getId());
            pilaRevisados.push(puntoPartida);
            puntoPartida->procesado = true;
            puntoPartida->visitado=true;
            INodo* primero =  puntoPartida->getInfo();
            result.push_back(primero);
            visitados++;
            ...
            for (int indiceArcos=0; indiceArcos<adyacentes->size(); ++indiceArcos) {
                        ...
                        if (!adyacente->procesado) {
                            pilaAdyasentes.push(adyacente);
                            adyacente->procesado = true;
                            //std::cout<<"Anadi: "<<adyacente->getInfo()->getId();
                        }
                    }
            do
            {
                while (!pilaAdyasentes.empty())
                {
                    NodoGrafo* actual = pilaAdyasentes.top();
                    pilaAdyasentes.pop();
                    ...
                    NodoGrafo* nodoProcesado = this->getNodo(actual->getInfo()->getId());
                    pilaRevisados.push(nodoProcesado);

                    INodo* nodoRevisado =  actual->getInfo();
                    result.push_back(nodoRevisado);
                    visitados++;
                    //std::cout<<"Anadi: "<<nodoRevisado->getId();
